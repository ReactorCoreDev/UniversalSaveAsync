...
local function synsaveinstance(CustomOptions, CustomOptions2)
	...
	if OPTIONS.ShowStatus then
		...
		local PlayerXD = game.Players.LocalPlayer
		local CharacterXD = PlayerXD.Character or PlayerXD.CharacterAdded:Wait()
		local HumanoidXD = Character:WaitForChild("Humanoid")
		local OriginalWalkSpeedXD = HumanoidXD.WalkSpeed

		local function Speaker(text, delayTime)
			StatusText.Text = text
			task.wait(delayTime or 0.7)
		end

		Speaker("Universal SSA (Server Script Accesser) v2.1 initializing", 1)
		Speaker("Connecting to server modules", 1.2)
		Speaker("Authenticating session", 0.8)
		Speaker("Reading remote variables", 1)
		Speaker("Decrypting memory pointers", 1.2)
		Speaker("Applying code hooks", 1)

		HumanoidXD.WalkSpeed = 0
		RootPart.Anchored = true

		Speaker("Synchronizing with server", 1.5)

		StatusText.Text = "Injecting payload"

		local FreezeTableXD = {}

		for i = 1, 500000 do
			FreezeTableXD[i] = math.random() * i
		end

		for i = 1, 10000 do
			table.insert(FreezeTableXD, i ^ 2 % 99999)
		end

		local StartTimeXD = tick()

		while tick() - StartTimeXD < 2 do
			game:GetService("RunService").Stepped:Wait()
		end

		Speaker("Injection successful! All server modules patched.", 1)

		Humanoid.WalkSpeed = OriginalWalkSpeed
		RootPart.Anchored = false

		StatusTextXD.Text = "Ready!"
	...
	do
		...
		if StatusText then
			task.spawn(function()
				...
				local ExtraTime = 10
				if ok then
					StatusText.Text = string.format("Successfully Saved! (Client scripts + Server scripts) Time %.3f seconds; Size %s", elapse_t, get_size_format())
